...
try:
    from scapy.all import RandBin, RandMAC, conf  # type: ignore
    ...
    class RandBin:  # minimal fallback
    ...
    class RandMAC:  # minimal fallback
...
# Scapy IPv6 address type constants (subset used here)
...
# Legacy comparator depends on external dst; kept for compatibility with documented behavior.
...
def get_source_addr_from_candidate_set(dst: str, candidate_set: List[str]) -> str:
    if not candidate_set:
        return ""
    D = ipaddress.IPv6Address(dst)

    def scope(addr: ipaddress.IPv6Address) -> str:
        ...
        if addr.packed[:2] >= b"\xfe\x80" and addr.packed[:2] <= b"\xfe\xbf" and (addr.packed[0] == 0xFE) and (
            (addr.packed[1] & 0xC0) == 0x80
        ):
            return "link"
    ...
    def common_prefix_len(a: ipaddress.IPv6Address, b: ipaddress.IPv6Address) -> int:
        x = int(a) ^ int(b)
        if x == 0:
            return 128
        return 128 - x.bit_length()

    def cmp(a_str: str, b_str: str) -> int:
        A = ipaddress.IPv6Address(a_str)
        B = ipaddress.IPv6Address(b_str)
        ...
        pa = common_prefix_len(A, D)
        pb = common_prefix_len(B, D)
        if pa > pb:
            return 1
        if pa < pb:
            return -1
...
class Net6(Net):
    ...
    def ip2int(cls, addr):
        ...
        b = socket.inet_pton(socket.AF_INET6, a)

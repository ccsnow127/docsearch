...
try:
    ...
    from scapy.utils import strand as _strand  # type: ignore
    from scapy.utils import stror as _stror  # type: ignore
    from scapy.utils import strxor as _strxor  # type: ignore
...
# Scapy IPv6 address type constants (values aligned with Scapy's usual ones)
...
def in6_getAddrType(addr: str) -> int:
    ...
    if (naddr[0] & 0xE0) == 0x20:
        ...
        if scope == 0x2:
            ...
            addrType |= IPV6_ADDR_GLOBAL
        else:
            addrType |= IPV6_ADDR_GLOBAL
    elif naddr[0] == 0xFE and (naddr[1] & 0xC0) == 0x80:
...
def teredoAddrExtractInfo(x: str) -> Tuple[str, int, str, int]:
    ...
    port_b = bytes([b ^ 0xFF for b in addr[10:12]])
...
dst = "::"  # destination used by rfc3484_cmp (captured externally in original)

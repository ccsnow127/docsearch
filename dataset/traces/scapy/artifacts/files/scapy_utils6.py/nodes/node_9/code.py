...
# --- Minimal Scapy-like compatibility layer (constants, conf, exceptions, Net) ---

class Scapy_Exception(Exception):
    pass


# Address type/scope constants (subset used by documented functions)
...
class _ManufDB:
    def _get_manuf(self, mac: str) -> str:
        # Minimal placeholder: unknown vendor
        return "UNKNOWN"
...
class RandMAC:
    def __init__(self):
        ...
        self._b = os.urandom(6)

    def __str__(self) -> str:
        return ":".join(f"{x:02x}" for x in self._b)
...
class Net:
    ...
    def name2addr(cls, addr: str) -> str:
        # Minimal: if it's already an IP literal, return it; else resolve.
        try:
...
def inet_pton(family: int, addr: str) -> bytes:
...
# --- bytewise ops (scapy.utils.strand/stror/strxor equivalents) ---
...
def in6_isincluded(addr: str, prefix: str, plen: int) -> bool:
    ...
    b_prefix = inet_pton(socket.AF_INET6, prefix)
    return in6_and(b_addr, mask) == b_prefix


def in6_isllsnmaddr(str: str) -> bool:
    a = inet_pton(socket.AF_INET6, str)
...
def in6_isaddrllallnodes(str: str) -> bool:
    return inet_pton(socket.AF_INET6, str) == inet_pton(socket.AF_INET6, "ff02::1")


def in6_isaddrllallservers(str: str) -> bool:
    return inet_pton(socket.AF_INET6, str) == inet_pton(socket.AF_INET6, "ff02::2")
...
def in6_get_common_plen(a, b) -> int:
    ba = inet_pton(socket.AF_INET6, a)
    bb = inet_pton(socket.AF_INET6, b)
...
def in6_get6to4Prefix(addr: str) -> Optional[str]:
    try:
        b = inet_pton(socket.AF_INET, addr)
    except Exception:
        return None
    v6 = b"\x20\x02" + b + b"\x00" * 10
    return inet_ntop(socket.AF_INET6, v6)
...
def in6_getLocalUniquePrefix() -> str:
    ...
    eui64_b = inet_pton(socket.AF_INET6, "::" + eui64_s)[8:16]
    digest = hashlib.sha1(btod + eui64_b).digest()
    gid = digest[:5]
    v6 = b"\xfd" + gid + b"\x00" * 10
    return inet_ntop(socket.AF_INET6, v6)
...
def in6_getRandomizedIfaceId(ifaceid: str, previous: Optional[str] = None) -> Tuple[str, str]:
    if previous is None:
        ...
        b_previous = inet_pton(socket.AF_INET6, "::" + previous)[8:16]
    b_ifaceid = inet_pton(socket.AF_INET6, "::" + ifaceid)[8:16]
    ...
    def half_to_str(half: bytes) -> str:
        full = b"\xff" * 8 + half
        return inet_ntop(socket.AF_INET6, full)[20:]
...
def in6_ctop(addr: str) -> Optional[str]:
    ...
    res.reverse()
    b = b"".join(res)
    return inet_ntop(socket.AF_INET6, b)
...
def in6_isaddr6to4(x: str) -> bool:
    b = inet_pton(socket.AF_INET6, x)
    return b[:2] == b" \x02"


def in6_isaddrTeredo(x: str) -> bool:
    bx = inet_pton(socket.AF_INET6, x)[:4]
    bp = inet_pton(socket.AF_INET6, conf.teredoPrefix)[:4]
    return bx == bp
...
def teredoAddrExtractInfo(x: str) -> Tuple[str, int, str, int]:
    ...
    mappedaddr = inet_ntop(socket.AF_INET, in6_xor(addr[12:16], b"\xff\xff\xff\xff"))
    return (server, flag, mappedaddr, mappedport)


def in6_iseui64(x: str) -> bool:
    s = inet_pton(socket.AF_INET6, "::ff:fe00:0")
...
def in6_isanycast(x: str) -> bool:
    if in6_iseui64(x):
        ...
        bx = inet_pton(socket.AF_INET6, x)
        bs = inet_pton(socket.AF_INET6, s)
...
def in6_getha(prefix: str) -> str:
    bprefix = inet_pton(socket.AF_INET6, prefix)
    net = in6_and(bprefix, in6_cidr2mask(64))
    res = in6_or(net, inet_pton(socket.AF_INET6, "::fdff:ffff:ffff:fffe"))
    return inet_ntop(socket.AF_INET6, res)
...
def rfc3484_cmp(source_a: str, source_b: str) -> int:
    ...
    if dst is None:
        raise AttributeError("rfc3484_cmp.dst is not set")
    if source_a == dst:
        return 1
    if source_b == dst:
        return 1
...
def get_source_addr_from_candidate_set(dst: str, candidate_set: List[str]) -> str:
    ...
    def appropriate(src: ipaddress.IPv6Address, dstv: ipaddress.IPv6Address) -> bool:
        # Minimal scope check: link-local must match link-local; otherwise non-link-local.
        return src.is_link_local == dstv.is_link_local

    def common_plen(a: ipaddress.IPv6Address, b: ipaddress.IPv6Address) -> int:
        x = int(a) ^ int(b)
        if x == 0:
            return 128
        return 128 - x.bit_length()

    parsed = {s: ipaddress.IPv6Address(s) for s in candidate_set}
    ...
    def cmp(a: str, b: str) -> int:
        ...
        if a_eq and not b_eq:
            return 1
        if b_eq and not a_eq:
            return -1

        a_app = appropriate(aa, dst_ip)
        b_app = appropriate(bb, dst_ip)
        if a_app and not b_app:
            return 1
        if b_app and not a_app:
            return -1

        pa = common_plen(aa, dst_ip)
        pb = common_plen(bb, dst_ip)
        if pa > pb:
            return 1
        if pb > pa:
            return -1
...
class Net6(Net):
    ...
    def ip2int(cls, addr):
        ...
        packed = inet_pton(socket.AF_INET6, a)
    ...
    def int2ip(val):
        ...
        return inet_ntop(socket.AF_INET6, packed)

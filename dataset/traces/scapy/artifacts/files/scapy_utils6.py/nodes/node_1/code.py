...
def construct_source_candidate_set(
    ...
    candidates: Iterator[Tuple[str, int, str]] = iter(())

    if in6_isgladdr(addr) or in6_isuladdr(addr):
        candidates = (t for t in laddr if t[1] == IPV6_ADDR_GLOBAL)
    elif in6_islladdr(addr):
        candidates = (t for t in laddr if t[1] == IPV6_ADDR_LINKLOCAL)
    elif in6_issladdr(addr):
        candidates = (t for t in laddr if t[1] == IPV6_ADDR_SITELOCAL)
    elif in6_ismaddr(addr):
        if in6_ismnladdr(addr):
            candidates = iter((("::1", IPV6_ADDR_LOOPBACK, conf.loopback_name),))
        ...
        candidates = (t for t in laddr if t[1] == IPV6_ADDR_LOOPBACK)
    else:
        candidates = iter(())

    res = [t[0] for t in candidates]

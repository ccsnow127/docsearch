...
class RandMAC:
    def __init__(self) -> None:
        self._b = bytes(random.getrandbits(8) for _ in range(6))
...
class RandBin:
    def __init__(self, n: int) -> None:
        self._b = bytes(random.getrandbits(8) for _ in range(n))
...
def in6_get6to4Prefix(addr: str) -> Optional[str]:
    try:
        v4 = inet_pton(AF_INET, addr)
    except Exception:
        return None
    v6 = b"\x20\x02" + v4 + b"\x00" * 10
...
def in6_getha(prefix: str) -> str:
    ...
    res = in6_or(net, inet_pton(AF_INET6, "::fdff:ffff:ffff:fffe"))
    return inet_ntop(AF_INET6, res)
...
def get_source_addr_from_candidate_set(dst: str, candidate_set: List[str]) -> str:
    ...
    def is_scope_appropriate(src: ipaddress.IPv6Address) -> bool:
        s = scope(src)
        if dst_scope == "link":
            return s == "link"
        if dst_scope == "unique_local":
            return s in ("unique_local", "global")
        return s == "global"
    ...
    def cmp(a: str, b: str) -> int:
        a_ip = ipaddress.IPv6Address(a)
        b_ip = ipaddress.IPv6Address(b)
        ...
        a_app = is_scope_appropriate(a_ip)
        b_app = is_scope_appropriate(b_ip)
        if a_app and not b_app:
            return 1
        if b_app and not a_app:
            return -1

        pa = common_plen(a_ip, dst_ip)
        pb = common_plen(b_ip, dst_ip)
        if pa > pb:
            return 1
        if pb > pa:
            return -1

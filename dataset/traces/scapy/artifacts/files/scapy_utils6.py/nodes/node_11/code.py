...
# Scapy-like IPv6 address type constants (bitflags)
...
IPV6_ADDR_LOOPBACK = 0x0080
IPV6_ADDR_UNSPECIFIED = 0x0100

IPV6_ADDR_6TO4 = 0x0200


def _inet_pton(family: int, addr: str) -> bytes:
...
def in6_ptop(str: str) -> str:
    return _inet_ntop(socket.AF_INET6, _inet_pton(socket.AF_INET6, str))


def in6_isincluded(addr: str, prefix: str, plen: int) -> bool:
    b_addr = _inet_pton(socket.AF_INET6, addr)
    mask = in6_cidr2mask(plen)
    b_prefix = _inet_pton(socket.AF_INET6, prefix)
    return in6_and(b_addr, mask) == b_prefix


def in6_isllsnmaddr(str: str) -> bool:
    a = _inet_pton(socket.AF_INET6, str)
...
def in6_isaddrllallnodes(str: str) -> bool:
    return _inet_pton(socket.AF_INET6, str) == _inet_pton(socket.AF_INET6, "ff02::1")


def in6_isaddrllallservers(str: str) -> bool:
    return _inet_pton(socket.AF_INET6, str) == _inet_pton(socket.AF_INET6, "ff02::2")
...
def in6_get_common_plen(a, b) -> int:
    ba = _inet_pton(socket.AF_INET6, a)
    bb = _inet_pton(socket.AF_INET6, b)
...
def in6_isvalid(address) -> bool:
    try:
        _inet_pton(socket.AF_INET6, address)
        return True
    except Exception:
        return False
...
def in6_getAddrType(addr: str) -> int:
    naddr = _inet_pton(socket.AF_INET6, addr)
    paddr = _inet_ntop(socket.AF_INET6, naddr)
    addrType = 0
    first_byte = naddr[0]
...
def in6_ifaceidtomac(ifaceid_s: str) -> Optional[str]:
    try:
        ifaceid = _inet_pton(socket.AF_INET6, "::" + ifaceid_s)[8:16]
    except Exception:
        return None
...
def in6_addrtomac(addr: str) -> Optional[str]:
    mask = _inet_pton(socket.AF_INET6, "::ffff:ffff:ffff:ffff")
    baddr = _inet_pton(socket.AF_INET6, addr)
    x = in6_and(mask, baddr)
    ifaceid = _inet_ntop(socket.AF_INET6, x)[2:]
...
def in6_getLinkScopedMcastAddr(
    ...
    return _inet_ntop(socket.AF_INET6, a)


def in6_get6to4Prefix(addr: str) -> Optional[str]:
    try:
        b4 = _inet_pton(socket.AF_INET, addr)
    except Exception:
        return None
    b6 = b"\x20\x02" + b4 + b"\x00" * 10
    return _inet_ntop(socket.AF_INET6, b6)
...
def in6_6to4ExtractAddr(addr: str) -> Optional[str]:
    ...
    if baddr[:2] != b" \x02":
        return None
    return _inet_ntop(socket.AF_INET, baddr[2:6])


def _rand_mac_str() -> str:
    b = bytes(random.getrandbits(8) for _ in range(6))
    return ":".join(f"{x:02x}" for x in b)


def _rand_bin(n: int) -> bytes:
...
def in6_getLocalUniquePrefix() -> str:
    ...
    eui64_b = _inet_pton(socket.AF_INET6, "::" + eui64_s)[8:16]
    digest = hashlib.sha1(btod + eui64_b).digest()
    global_id = digest[:5]
    b6 = b"\xfd" + global_id + b"\x00" * 10
    return _inet_ntop(socket.AF_INET6, b6)


def in6_getRandomizedIfaceId(ifaceid: str, previous: Optional[str] = None) -> Tuple[str, str]:
    if previous is None:
        b_previous = bytes(_rand_bin(8))
    else:
        b_previous = _inet_pton(socket.AF_INET6, "::" + previous)[8:16]
    b_ifaceid = _inet_pton(socket.AF_INET6, "::" + ifaceid)[8:16]
    ...
    def _half_to_str(half: bytes) -> str:
        full = b"\xff" * 8 + half
        p = _inet_ntop(socket.AF_INET6, full)
        return p[20:]
...
def in6_ctop(addr: str) -> Optional[str]:
    ...
    for _ in range(4):
        ...
        i //= 2**32
    res.reverse()
    return _inet_ntop(socket.AF_INET6, b"".join(res))


def in6_ptoc(addr: str) -> Optional[str]:
    try:
        d = struct.unpack("!IIII", _inet_pton(socket.AF_INET6, addr))
    except Exception:
        return None
    m = [2**96, 2**64, 2**32, 1]
    rem = 0
...
def in6_isaddr6to4(x: str) -> bool:
    b = _inet_pton(socket.AF_INET6, x)
    return b[:2] == b" \x02"


def in6_isaddrTeredo(x: str) -> bool:
    bx = _inet_pton(socket.AF_INET6, x)[:4]
    bp = _inet_pton(socket.AF_INET6, conf.teredoPrefix)[:4]
    return bx == bp


def teredoAddrExtractInfo(x: str) -> Tuple[str, int, str, int]:
    addr = _inet_pton(socket.AF_INET6, x)
    server = _inet_ntop(socket.AF_INET, addr[4:8])
    flag = struct.unpack("!H", addr[8:10])[0]
    mappedport = struct.unpack("!H", in6_xor(addr[10:12], b"\xff\xff"))[0]
    mappedaddr = _inet_ntop(socket.AF_INET, in6_xor(addr[12:16], b"\xff\xff\xff\xff"))
    return (server, flag, mappedaddr, mappedport)


def in6_iseui64(x: str) -> bool:
    c = _inet_pton(socket.AF_INET6, "::ff:fe00:0")
    bx = _inet_pton(socket.AF_INET6, x)
    return in6_and(bx, c) == c
...
def in6_isanycast(x: str) -> bool:
    if in6_iseui64(x):
        ...
        bx = _inet_pton(socket.AF_INET6, x)
        bs = _inet_pton(socket.AF_INET6, s)
...
def in6_getnsma(a: bytes) -> bytes:
    r = in6_and(a, _inet_pton(socket.AF_INET6, "::ff:ffff"))
    r = in6_or(_inet_pton(socket.AF_INET6, "ff02::1:ff00:0"), r)
    return r
...
def in6_getnsmac(a: bytes) -> str:
    ...
    last4 = d[-4:]
    return "33:33:" + ":".join("%.2x" % b for b in last4)


def in6_getha(prefix: str) -> str:
    bprefix = _inet_pton(socket.AF_INET6, prefix)
    net = in6_and(bprefix, in6_cidr2mask(64))
    ha = in6_or(net, _inet_pton(socket.AF_INET6, "::fdff:ffff:ffff:fffe"))
    return _inet_ntop(socket.AF_INET6, ha)
...
dst = None  # external destination placeholder for rfc3484_cmp compatibility
...
def get_source_addr_from_candidate_set(dst: str, candidate_set: List[str]) -> str:
    ...
    if "%" in dst:
        ipaddress.IPv6Address(dst)  # will raise
    dst_ip = ipaddress.IPv6Address(dst)

    parsed_cache: dict[str, ipaddress.IPv6Address] = {}

    def parse(s: str) -> ipaddress.IPv6Address:
        if "%" in s:
            return ipaddress.IPv6Address(s)  # raise
        v = parsed_cache.get(s)
        if v is None:
            v = ipaddress.IPv6Address(s)
            parsed_cache[s] = v
        return v

    ula_net = ipaddress.IPv6Network("fc00::/7")

    def scope_class(ip: ipaddress.IPv6Address) -> str:
        if ip.is_loopback:
            return "loopback"
        if ip.is_link_local:
            return "link_local"
    ...
    dst_scope = scope_class(dst_ip)

    def scope_appropriate(src_scope: str) -> Optional[bool]:
        if dst_scope == "loopback":
            return src_scope == "loopback"
        if dst_scope == "link_local":
            return src_scope == "link_local"
        if dst_scope == "multicast":
            if src_scope == "loopback":
                return False
            if src_scope in ("link_local", "unique_local", "global"):
                return True
            return False
        if dst_scope == "unique_local":
            if src_scope in ("unique_local", "global"):
                return True
            return False
        if dst_scope == "global":
            if src_scope in ("global", "unique_local"):
                return True
            return False
        return None  # dst_scope == "other"
    ...
    def cmp(a: str, b: str) -> int:
        ...
        a_scope = scope_class(a_ip)
        b_scope = scope_class(b_ip)
        a_app = scope_appropriate(a_scope)
        b_app = scope_appropriate(b_scope)
        if a_app is not None and b_app is not None and a_app != b_app:
            return 1 if a_app else -1

        pa = common_prefix_len(int(a_ip), int(dst_ip))
        pb = common_prefix_len(int(b_ip), int(dst_ip))
        if pa > pb:
            return 1
        if pa < pb:
            return -1
...
class Net:
    ...
    def name2addr(cls, addr: str) -> str:
        try:
            infos = socket.getaddrinfo(addr, None, cls.family, socket.SOCK_STREAM)
            if infos:
                return infos[0][4][0]
        except Exception:
            pass
...
class Net6(Net):
    ...
    def ip2int(cls, addr: str) -> int:
        ...
        packed = _inet_pton(socket.AF_INET6, addr)
    ...
    def int2ip(val: int) -> str:
        ...
        return _inet_ntop(socket.AF_INET6, packed)

...
try:
    ...
    class RandMAC:  # type: ignore
        def __str__(self) -> str:
            b = os.urandom(6)
            return ":".join(f"{x:02x}" for x in b)

    class RandBin:  # type: ignore
...
# Scapy IPv6 address-type constants (exact required numeric values)
IPV6_ADDR_UNICAST = 0x01
IPV6_ADDR_MULTICAST = 0x02
IPV6_ADDR_6TO4 = 0x04
IPV6_ADDR_UNSPECIFIED = 0x08
IPV6_ADDR_LOOPBACK = 0x10
IPV6_ADDR_LINKLOCAL = 0x20
IPV6_ADDR_SITELOCAL = 0x40  # not specified in required list; used by scope_cmp/in6_getscope
...
def in6_ptop(str: str) -> str:
    b = socket.inet_pton(AF_INET6, str)
    return socket.inet_ntop(AF_INET6, b)
...
def in6_isincluded(addr: str, prefix: str, plen: int) -> bool:
    b_addr = socket.inet_pton(AF_INET6, addr)
    mask = in6_cidr2mask(plen)
    b_prefix = socket.inet_pton(AF_INET6, prefix)
    return in6_and(b_addr, mask) == b_prefix


def in6_isllsnmaddr(str: str) -> bool:
    a = socket.inet_pton(AF_INET6, str)
...
def in6_isaddrllallnodes(str: str) -> bool:
    c = socket.inet_pton(AF_INET6, "ff02::1")
    a = socket.inet_pton(AF_INET6, str)
    return a == c


def in6_isaddrllallservers(str: str) -> bool:
    c = socket.inet_pton(AF_INET6, "ff02::2")
    a = socket.inet_pton(AF_INET6, str)
    return a == c
...
def in6_get_common_plen(a, b) -> int:
    ...
    ba = socket.inet_pton(AF_INET6, a)
    bb = socket.inet_pton(AF_INET6, b)
    for i in range(16):
...
def in6_isaddr6to4(x: str) -> bool:
    b = socket.inet_pton(AF_INET6, x)
    return b[:2] == b" \x02"


def in6_isaddrTeredo(x: str) -> bool:
    bx = socket.inet_pton(AF_INET6, x)[:4]
    bp = socket.inet_pton(AF_INET6, conf.teredoPrefix)[:4]
    return bx == bp


def teredoAddrExtractInfo(x: str) -> Tuple[str, int, str, int]:
    addr = socket.inet_pton(AF_INET6, x)
    server = socket.inet_ntop(AF_INET, addr[4:8])
    flag = struct.unpack("!H", addr[8:10])[0]
    mappedport = struct.unpack("!H", in6_xor(addr[10:12], b"\xff\xff"))[0]
    mappedaddr = socket.inet_ntop(AF_INET, in6_xor(addr[12:16], b"\xff\xff\xff\xff"))
    return (server, flag, mappedaddr, mappedport)


def in6_iseui64(x: str) -> bool:
    s = socket.inet_pton(AF_INET6, "::ff:fe00:0")
    a = socket.inet_pton(AF_INET6, x)
    return in6_and(a, s) == s


def in6_isanycast(x: str) -> bool:
    if in6_iseui64(x):
        s = "::fdff:ffff:ffff:ff80"
        bx = socket.inet_pton(AF_INET6, x)
        bs = socket.inet_pton(AF_INET6, s)
...
def in6_getnsma(a: bytes) -> bytes:
    r = in6_and(a, socket.inet_pton(AF_INET6, "::ff:ffff"))
    r = in6_or(socket.inet_pton(AF_INET6, "ff02::1:ff00:0"), r)
    return r
...
def in6_getnsmac(a: bytes) -> str:
    ...
    last4 = d[-4:]
    return "33:33:" + ":".join(f"{x:02x}" for x in last4)


def in6_getha(prefix: str) -> str:
    p = socket.inet_pton(AF_INET6, prefix)
    net = in6_and(p, in6_cidr2mask(64))
    ha = in6_or(net, socket.inet_pton(AF_INET6, "::fdff:ffff:ffff:fffe"))
    return socket.inet_ntop(AF_INET6, ha)


def in6_getAddrType(addr: str) -> int:
    naddr = socket.inet_pton(AF_INET6, addr)
    paddr = socket.inet_ntop(AF_INET6, naddr)
    addrType = 0
    if (naddr[0] & 0xE0) == 0x20:
        ...
        if len(paddr) > 3 and paddr[3] in ("2",):
            addrType = IPV6_ADDR_LINKLOCAL | IPV6_ADDR_MULTICAST
        elif len(paddr) > 3 and paddr[3] in ("e", "E"):
...
def in6_ifaceidtomac(ifaceid_s: str) -> Optional[str]:
    try:
        ifaceid = socket.inet_pton(AF_INET6, "::" + ifaceid_s)[8:16]
    except Exception:
        return None
...
def in6_addrtomac(addr: str) -> Optional[str]:
    mask = socket.inet_pton(AF_INET6, "::ffff:ffff:ffff:ffff")
    baddr = socket.inet_pton(AF_INET6, addr)
    x = in6_and(mask, baddr)
    ifaceid = socket.inet_ntop(AF_INET6, x)[2:]
...
def in6_getLinkScopedMcastAddr(
    ...
    return socket.inet_ntop(AF_INET6, a)


def in6_get6to4Prefix(addr: str) -> Optional[str]:
    try:
        b4 = socket.inet_pton(AF_INET, addr)
    except Exception:
        return None
    b6 = b"\x20\x02" + b4 + b"\x00" * 10
    return socket.inet_ntop(AF_INET6, b6)
...
def in6_6to4ExtractAddr(addr: str) -> Optional[str]:
    ...
    if baddr[:2] != b" \x02":
        return None
    return socket.inet_ntop(AF_INET, baddr[2:6])
...
def in6_getLocalUniquePrefix() -> str:
    ...
    eui64_b = socket.inet_pton(AF_INET6, "::" + eui64_s)[8:16]
...
def in6_getRandomizedIfaceId(ifaceid: str, previous: Optional[str] = None) -> Tuple[str, str]:
    if previous is None:
        ...
        b_previous = socket.inet_pton(AF_INET6, "::" + previous)[8:16]
    b_ifaceid = socket.inet_pton(AF_INET6, "::" + ifaceid)[8:16]
    ...
    def half_to_str(h: bytes) -> str:
        a = b"\xff" * 8 + h
        p = socket.inet_ntop(AF_INET6, a)
        return p[20:]

    return (half_to_str(bytes(s1)), half_to_str(s2))
...
def in6_ptoc(addr: str) -> Optional[str]:
    try:
        d = struct.unpack("!IIII", socket.inet_pton(AF_INET6, addr))
    except Exception:
        return None
    m = [2**96, 2**64, 2**32, 1]
    rem = 0
...
_dst_for_rfc3484_cmp: Optional[str] = None


def rfc3484_cmp(source_a: str, source_b: str) -> int:
    global _dst_for_rfc3484_cmp
    if _dst_for_rfc3484_cmp is None:
        raise ValueError("dst not set for rfc3484_cmp")
...
def get_source_addr_from_candidate_set(dst: str, candidate_set: List[str]) -> str:
    ...
    dst_ip = int.from_bytes(socket.inet_pton(AF_INET6, dst), "big")

    def scope(addr_s: str) -> str:
        ip = socket.inet_pton(AF_INET6, addr_s)
        v = int.from_bytes(ip, "big")
        if v == 1:
            return "node"
        if (v & ((1 << 128) - (1 << 118))) == int.from_bytes(socket.inet_pton(AF_INET6, "fe80::"), "big"):
            return "link"
        if (v & ((1 << 128) - (1 << 121))) == int.from_bytes(socket.inet_pton(AF_INET6, "fc00::"), "big"):
            return "unique_local"
        if (v & ((1 << 128) - (1 << 120))) == int.from_bytes(socket.inet_pton(AF_INET6, "ff00::"), "big"):
            if (v & ((1 << 128) - (1 << 112))) == int.from_bytes(socket.inet_pton(AF_INET6, "ff02::"), "big"):
                return "multicast_link"
            if (v & ((1 << 128) - (1 << 112))) == int.from_bytes(socket.inet_pton(AF_INET6, "ff0e::"), "big"):
                return "multicast_global"
            return "multicast_other"
        return "global"

    def is_scope_appropriate(src_scope: str, dst_scope: str) -> bool:
        if src_scope == "unknown":
            return False
        if dst_scope == "unknown":
            return False
        if dst_scope == "node":
            return src_scope == "node"
        if dst_scope in ("link", "multicast_link"):
            return src_scope == "link"
        if dst_scope == "unique_local":
            return src_scope in ("unique_local", "global")
        if dst_scope in ("global", "multicast_global"):
            return src_scope == "global"
        if dst_scope == "multicast_other":
            return src_scope == "global"
        return False

    dst_scope = scope(dst)

    def common_plen_int(a: int, b: int) -> int:
        x = a ^ b
        if x == 0:
            return 128
    ...
    def cmp(a: str, b: str) -> int:
        a_ip = int.from_bytes(socket.inet_pton(AF_INET6, a), "big")
        b_ip = int.from_bytes(socket.inet_pton(AF_INET6, b), "big")
...
class Net6(Net):
    ...
    def ip2int(cls, addr):
        ...
        b = socket.inet_pton(AF_INET6, a)
        val1, val2 = struct.unpack("!QQ", b)
        return (val1 << 64) + val2

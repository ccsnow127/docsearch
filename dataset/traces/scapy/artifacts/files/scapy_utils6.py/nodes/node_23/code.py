...
try:
    ...
    # Scapy canonical values (as used by Scapy's IPv6 address type flags)
    ...
    def strand(a: bytes, b: bytes) -> bytes:
        return bytes((x & y) for x, y in zip(a, b))

    def stror(a: bytes, b: bytes) -> bytes:
        return bytes((x | y) for x, y in zip(a, b))
...
def in6_getAddrType(addr: str) -> int:
    ...
    if (naddr[0] & 0xE0) == 0x20:
        ...
        elif len(paddr) > 3 and paddr[3] in "eE":
...
# Destination captured for rfc3484_cmp compatibility with original context.
...
def get_source_addr_from_candidate_set(dst: str, candidate_set: List[str]) -> str:
    ...
    def scope(addr_s: str) -> str:
        ...
        first = a[0]
        if addr_s == "::1" or a == socket.inet_pton(socket.AF_INET6, "::1"):
            return "node"
        ...
        if (a[0] & 0xFE) == 0xFC:
            return "unique_local"
        if first == 0xFF:
            # multicast scope is low nibble of second byte
            sc = a[1] & 0x0F
            if sc == 0x2:
        ...
        # treat remaining unicast as global
        return "global"
    ...
    def cmp(a: str, b: str) -> int:
        ...
        pa = common_plen_int(a_int, dst_int)
        pb = common_plen_int(b_int, dst_int)
        if pa > pb:
            return 1
        if pa < pb:
            return -1

...
try:
    ...
    def _binop(a1: bytes, a2: bytes, op):
        return bytes(op(x, y) for x, y in zip(a1, a2))

    def strand(a1: bytes, a2: bytes) -> bytes:
        return _binop(a1, a2, lambda x, y: x & y)
...
def in6_getRandomizedIfaceId(ifaceid: str, previous: Optional[str] = None) -> Tuple[str, str]:
    ...
    def _half_to_str(half: bytes) -> str:
        full = b"\xff" * 8 + half
        return socket.inet_ntop(socket.AF_INET6, full)[20:]
...
def get_source_addr_from_candidate_set(dst: str, candidate_set: List[str]) -> str:
    ...
    dst_norm = socket.inet_ntop(socket.AF_INET6, dst_ip)

    def _scope_rank_unicast(ip_packed: bytes, ip_norm: str) -> int:
        if ip_norm in ("::", "::1"):
            return 0
        if ip_packed[0] == 0xFF:
            return 3
        if in6_islladdr(ip_norm):
            return 1
        try:
            if in6_issladdr(ip_norm):
                return 2
        except Exception:
            return 3
        return 3

    def _required_rank(dst_packed: bytes, dst_norm_s: str) -> int:
        if dst_packed[0] == 0xFF:
            scope_val = dst_packed[1] & 0x0F
            if scope_val == 0x1:
                return 0
            if scope_val == 0x2:
                return 1
            ...
            if scope_val == 0xE:
                return 3
            return 3
        return _scope_rank_unicast(dst_packed, dst_norm_s)

    req_rank = _required_rank(dst_ip, dst_norm)

    def _common_prefix_len(a_p: bytes, b_p: bytes) -> int:
        for i in range(16):
            xa = a_p[i]
            xb = b_p[i]
            if xa == xb:
    ...
    def cmp(a: str, b: str) -> int:
        a_p = socket.inet_pton(socket.AF_INET6, a)
        b_p = socket.inet_pton(socket.AF_INET6, b)
        a_n = socket.inet_ntop(socket.AF_INET6, a_p)
        b_n = socket.inet_ntop(socket.AF_INET6, b_p)
        ...
        a_rank = _scope_rank_unicast(a_p, a_n)
        b_rank = _scope_rank_unicast(b_p, b_n)
        a_app = a_rank >= req_rank
        b_app = b_rank >= req_rank
        if a_app and not b_app:
            return 1
        if not a_app and b_app:
            return -1

        pa = _common_prefix_len(a_p, dst_ip)
        pb = _common_prefix_len(b_p, dst_ip)
        if pa > pb:
            return 1
        if pa < pb:
            return -1

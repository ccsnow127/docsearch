...
def _fallback_strxor(a: bytes, b: bytes) -> bytes:
    return bytes((x ^ y) for x, y in zip(a, b))


def _fallback_strand(a: bytes, b: bytes) -> bytes:
    return bytes((x & y) for x, y in zip(a, b))


def _fallback_stror(a: bytes, b: bytes) -> bytes:
    return bytes((x | y) for x, y in zip(a, b))


try:
    from scapy.utils import strxor as _scapy_strxor  # type: ignore
    from scapy.utils import strand as _scapy_strand  # type: ignore
    from scapy.utils import stror as _scapy_stror  # type: ignore
except Exception:  # pragma: no cover
    _scapy_strxor = _fallback_strxor
    _scapy_strand = _fallback_strand
    _scapy_stror = _fallback_stror


def _load_scapy_ipv6_constants():
    for modname in ("scapy.data", "scapy.layers.inet6"):
        try:
...
_scapy_const_mod = _load_scapy_ipv6_constants()

# Fallback values must match Scapy's canonical ones for the pinned version.
# These values match Scapy's long-standing inet6 constants layout.
_FALLBACK_IPV6_ADDR_UNICAST = 0x01
_FALLBACK_IPV6_ADDR_MULTICAST = 0x02
_FALLBACK_IPV6_ADDR_GLOBAL = 0x10
_FALLBACK_IPV6_ADDR_LINKLOCAL = 0x20
_FALLBACK_IPV6_ADDR_SITELOCAL = 0x40
_FALLBACK_IPV6_ADDR_LOOPBACK = 0x80
_FALLBACK_IPV6_ADDR_UNSPECIFIED = 0x100
_FALLBACK_IPV6_ADDR_6TO4 = 0x200


def _get_const(name: str, fallback: int) -> int:
    if _scapy_const_mod is not None and hasattr(_scapy_const_mod, name):
        try:
            return int(getattr(_scapy_const_mod, name))
        except Exception:
            return fallback
    return fallback


IPV6_ADDR_UNICAST = _get_const("IPV6_ADDR_UNICAST", _FALLBACK_IPV6_ADDR_UNICAST)
IPV6_ADDR_MULTICAST = _get_const("IPV6_ADDR_MULTICAST", _FALLBACK_IPV6_ADDR_MULTICAST)
IPV6_ADDR_GLOBAL = _get_const("IPV6_ADDR_GLOBAL", _FALLBACK_IPV6_ADDR_GLOBAL)
IPV6_ADDR_LINKLOCAL = _get_const("IPV6_ADDR_LINKLOCAL", _FALLBACK_IPV6_ADDR_LINKLOCAL)
IPV6_ADDR_SITELOCAL = _get_const("IPV6_ADDR_SITELOCAL", _FALLBACK_IPV6_ADDR_SITELOCAL)
IPV6_ADDR_LOOPBACK = _get_const("IPV6_ADDR_LOOPBACK", _FALLBACK_IPV6_ADDR_LOOPBACK)
IPV6_ADDR_UNSPECIFIED = _get_const("IPV6_ADDR_UNSPECIFIED", _FALLBACK_IPV6_ADDR_UNSPECIFIED)
IPV6_ADDR_6TO4 = _get_const("IPV6_ADDR_6TO4", _FALLBACK_IPV6_ADDR_6TO4)
...
def in6_or(a1: bytes, a2: bytes) -> bytes:
    return _scapy_stror(a1, a2)


def in6_and(a1: bytes, a2: bytes) -> bytes:
    return _scapy_strand(a1, a2)


def in6_xor(a1: bytes, a2: bytes) -> bytes:
    return _scapy_strxor(a1, a2)
...
def in6_ptop(str: str) -> str:
    b = socket.inet_pton(socket.AF_INET6, str)
...
def in6_get6to4Prefix(addr: str) -> Optional[str]:
    try:
        b = socket.inet_pton(socket.AF_INET, addr)
    except Exception:
        return None
...
def in6_isaddrTeredo(x: str) -> bool:
    ...
    bp = socket.inet_pton(socket.AF_INET6, conf.teredoPrefix)[:4]  # type: ignore[attr-defined]
    return bx == bp
...
def in6_getha(prefix: str) -> str:
    p = socket.inet_pton(socket.AF_INET6, prefix)
...
def construct_source_candidate_set(addr, plen, laddr):
    ...
    if in6_isgladdr(addr) or in6_isuladdr(addr):
        ...
        if in6_ismnladdr(addr):
            cset_iter = iter([("::1", 16, conf.loopback_name)])  # type: ignore[attr-defined]
...
# Destination used by rfc3484_cmp (captured equivalently to original context)
dst: str = "::"
...
def get_source_addr_from_candidate_set(dst: str, candidate_set: List[str]) -> str:
    ...
    def scope(addr_s: str) -> str:
        ip = int.from_bytes(socket.inet_pton(socket.AF_INET6, addr_s), "big")
        if ip == 1:
            return "node"
        if (ip & ((1 << 128) - (1 << 118))) == (int.from_bytes(socket.inet_pton(socket.AF_INET6, "fe80::"), "big") & ((1 << 128) - (1 << 118))):
            return "link"
        if (ip & ((1 << 128) - (1 << 121))) == (int.from_bytes(socket.inet_pton(socket.AF_INET6, "fc00::"), "big") & ((1 << 128) - (1 << 121))):
            return "unique_local"
        if (ip & (0xFF << 120)) == (0xFF << 120):
            if (ip & (0xFFFF << 112)) == (0xFF02 << 112):
                return "multicast_link"
            if (ip & (0xFFFF << 112)) == (0xFF0E << 112):
    ...
    def cmp(a: str, b: str) -> int:
        a_int = int.from_bytes(socket.inet_pton(socket.AF_INET6, a), "big")
        b_int = int.from_bytes(socket.inet_pton(socket.AF_INET6, b), "big")
        ...
        pa = common_plen_int(a_int, dst_ip)
        pb = common_plen_int(b_int, dst_ip)
        if pa > pb:
            return 1
        if pa < pb:
            return -1

...
try:
    ...
    # Best-effort constants (values not guaranteed to match Scapy)
...
def construct_source_candidate_set(
    ...
    if in6_isgladdr(addr) or in6_isuladdr(addr):
        it = (x for x in laddr if x[1] == IPV6_ADDR_GLOBAL)
    elif in6_islladdr(addr):
        it = (x for x in laddr if x[1] == IPV6_ADDR_LINKLOCAL)
    elif in6_issladdr(addr):
        it = (x for x in laddr if x[1] == IPV6_ADDR_SITELOCAL)
    elif in6_ismaddr(addr):
        if in6_ismnladdr(addr):
            it = iter([("::1", IPV6_ADDR_LOOPBACK, conf.loopback_name)])
        ...
        it = (x for x in laddr if x[1] == IPV6_ADDR_LOOPBACK)
    else:
        it = iter([])
...
def rfc3484_cmp(source_a: str, source_b: str) -> int:
    ...
    if dst is None:
        raise ValueError("Destination not set for rfc3484_cmp")
    if source_a == dst:
        return 1
    if source_b == dst:
        return 1

...
try:
    ...
    class RandBin(bytes):
        def __new__(cls, n):
            ...
            return bytes.__new__(cls, os.urandom(n))
    ...
    class RandMAC(str):
        def __new__(cls):
            ...
            b = os.urandom(6)
            return str.__new__(cls, ":".join(f"{x:02x}" for x in b))
...
def rfc3484_cmp(source_a: str, source_b: str) -> int:
    ...
    if dst is None:
        raise ValueError("rfc3484_cmp.dst must be set to destination address")
    if source_a == dst:
        return 1
    if source_b == dst:
        return 1
...
def get_source_addr_from_candidate_set(dst: str, candidate_set: List[str]) -> str:
    ...
    def _scope_appropriate(src: str, dst_: str) -> bool:
        s_src = in6_getscope(src)
        s_dst = in6_getscope(dst_)
        if s_src == -1:
            s_src = IPV6_ADDR_LOOPBACK
        if s_dst == -1:
            s_dst = IPV6_ADDR_LOOPBACK
        return scope_cmp(src, dst_) != -1
    ...
    def _cmp(a: str, b: str) -> int:
        ...
        if b == dst and a != dst:
            return -1
        aa = _scope_appropriate(a, dst)
        bb = _scope_appropriate(b, dst)
        if aa and not bb:
            return 1
        if bb and not aa:
            return -1
        pa = in6_get_common_plen(a, dst)
        pb = in6_get_common_plen(b, dst)
        if pa > pb:
            return 1
        if pb > pa:
            return -1

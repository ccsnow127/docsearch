...
dst = None  # used by rfc3484_cmp to mirror original external capture
...
def get_source_addr_from_candidate_set(dst: str, candidate_set: List[str]) -> str:
    ...
    def scope(addr_s: str) -> str:
        ...
        if a == socket.inet_pton(socket.AF_INET6, "::1"):
            return "node"
        if in6_islladdr(addr_s):
            return "link"
        if in6_isuladdr(addr_s):
            return "unique_local"
        if in6_ismaddr(addr_s):
            if in6_ismlladdr(addr_s):
                return "multicast_link"
            if in6_ismgladdr(addr_s):

# scapy Baseline Documentation

## Module: utils6

**purpose** This module provides IPv6-focused utilities used by Scapy for address classification, normalization, bitwise/prefix operations, special-address derivations (e.g., solicited-node multicast, 6to4, Teredo), and limited RFC-based source-address selection. It works primarily with IPv6 addresses in printable form (`str`) and network form (`bytes`), relying on `inet_pton/inet_ntop`, Scapy’s `conf`, and IPv6 type/scope constants from `scapy.data`.

**classes**
- **Net6** Extends `scapy.base_classes.Net` to represent IPv6 networks (up to /128), implementing `ip2int` and `int2ip` conversions between printable IPv6 addresses/hostnames and 128-bit integers for network arithmetic and iteration.

**key functions and relationships**
- **construct_source_candidate_set** filters an interface’s assigned addresses (tuples including an `IPV6_ADDR_*` type) into a candidate source list matching the destination’s scope, then sorts candidates to prefer global/native over 6to4 when applicable.
- **get_source_addr_from_candidate_set** selects the best source from that candidate list using a limited RFC 3484 comparison: it uses **scope_cmp** (based on **in6_getscope**) and **rfc3484_cmp** (which also uses **in6_get_common_plen**) to rank candidates.
- **in6_getAddrType** classifies an IPv6 address into Scapy’s bitmask type flags (unicast/multicast plus scope, and 6to4 marking), while the `in6_is*` predicates (e.g., **in6_islladdr**, **in6_isgladdr**, **in6_ismaddr**) are thin wrappers around **in6_isincluded** for common prefixes and are reused by scope/type logic.
- Address construction/extraction helpers include EUI-64 and MAC/vendor utilities (**in6_mactoifaceid**, **in6_ifaceidtomac**, **in6_addrtomac**, **in6_addrtovendor**), transition mechanisms (**in6_get6to4Prefix**, **in6_6to4ExtractAddr**, **in6_isaddr6to4**, **in6_isaddrTeredo**, **teredoAddrExtractInfo**), and multicast/ND helpers (**in6_getLinkScopedMcastAddr**, **in6_getnsma**, **in6_getnsmac**).
- Low-level primitives (**in6_or**, **in6_and**, **in6_xor**, **in6_cidr2mask**, **in6_mask2cidr**, **in6_ptop**, **in6_ctop**, **in6_ptoc**, **in6_isvalid**) support the higher-level operations by normalizing addresses, converting representations, and performing prefix/bitwise computations.

...
def pass_context(f: F) -> F:
    setattr(f, "jinja_pass_arg", _PassArg.context)
    return f


def pass_eval_context(f: F) -> F:
    setattr(f, "jinja_pass_arg", _PassArg.eval_context)
    return f


def pass_environment(f: F) -> F:
    setattr(f, "jinja_pass_arg", _PassArg.environment)
    return f
...
def generate_lorem_ipsum(n: int = 5, html: bool = True, min: int = 20, max: int = 100) -> "markupsafe.Markup | str":
    import random
...
class LRUCache:
    __copy__ = _copy.copy  # type: ignore[attr-defined]
    ...
    def __getitem__(self, key: t.Any) -> t.Any:
        with self._wlock:
            ...
            if not self._queue or self._queue[-1] != key:
                try:
                    self._remove(key)
                except ValueError:
                    pass

...
def internal_code(f: F) -> F:
    return internalcode(f)
...
def generate_lorem_ipsum(
    ...
    paragraphs = [f"<p>{markupsafe.escape(x)}</p>" for x in result]
...
def autoescape(value: t.Any) -> markupsafe.Markup:
    """Convert the value to Markup if it isn't already."""
    return value if isinstance(value, markupsafe.Markup) else markupsafe.Markup(value)

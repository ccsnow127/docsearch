...
_http_re = re.compile(
    r"^(?:https?://|www\.)"
    r"(?:[a-zA-Z0-9\-._~%!$&'()*+,;=]+@)?"
    r"(?:\[[0-9a-fA-F:.]+\]|[a-zA-Z0-9.-]+\.[a-zA-Z]{2,})"
    r"(?::\d{1,5})?"
    r"(?:/[^\s<]*)?$"
)
...
def generate_lorem_ipsum(n: int = 5, html: bool = True, min: int = 20, max: int = 100) -> "markupsafe.Markup | str":
    ...
    paragraphs = [f"<p>{markupsafe.escape(p)}</p>" for p in result]
...
def autoescape(template_name: str | None) -> bool:
    """Default autoescape callback.

    This is a minimal implementation that matches the documented entity list.
    """

    return select_autoescape()(template_name)

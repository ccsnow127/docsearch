...
def pass_environment(f: F) -> F:
    """Decorator for Jinja filters and other callables to receive an Environment.

    When a callable is decorated with this, Jinja will pass an Environment
    instance as the first positional argument when invoking it during template
    rendering (for example when used as a filter).
    """
...
def generate_lorem_ipsum(n: int = 5, html: bool = True, min: int = 20, max: int = 100) -> str:
    ...
    if n <= 0:
        return markupsafe.Markup("") if html else ""
...
def url_quote(obj: t.Any, charset: str = "utf-8", for_qs: bool = False) -> str:
    ...
    rv = quote_from_bytes(obj, safe)  # type: ignore[arg-type]
...
def autoescape(template_name: str | None, *, enabled_extensions: t.Collection[str] = ("html", "htm", "xml"), disabled_extensions: t.Collection[str] = (), default_for_string: bool = True, default: bool = False) -> bool:
...
def select_autoescape(
    ...
    def autoescape(template_name: str | None) -> bool:
        ...
        if name.endswith(enabled_patterns):
            return True

        if name.endswith(disabled_patterns):
            return False

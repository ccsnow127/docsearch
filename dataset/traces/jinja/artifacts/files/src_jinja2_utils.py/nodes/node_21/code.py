...
def pass_environment(f: F) -> F:
    """Decorator that marks a callable to receive an Environment instance.

    When a callable decorated with ``pass_environment`` is invoked by Jinja
    during template rendering (commonly filters, tests, and globals), an
    ``Environment`` object is passed as the first positional argument.
    """
...
def urlize(
    ...
    for idx, word in enumerate(words):
        ...
        if _http_re.fullmatch(middle):
        ...
        elif middle.startswith("mailto:") and _email_re.fullmatch(middle[7:]):
            ...
            and ":" not in middle
            and _email_re.fullmatch(middle)
        ):
...
def generate_lorem_ipsum(n: int = 5, html: bool = True, min: int = 20, max: int = 100) -> str:
    ...
    rv = "\n".join(f"<p>{markupsafe.escape(p)}</p>" for p in result)

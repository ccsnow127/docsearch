# utils Baseline Documentation

## Module: utils

**Purpose**: Provides core runtime utilities used across Jinja, including decorators that control what implicit rendering state is passed to callables, small helper functions for importing, caching, escaping, and formatting, plus template-facing helper objects (cycler/joiner/namespace) and an LRU cache implementation.

**Key classes**
- **_MissingType**: Implements the `missing` singleton used to represent an explicit “value not provided” sentinel, with stable `__repr__` and pickling behavior via `__reduce__`.
- **_PassArg**: An `enum.Enum` that encodes which implicit argument (`Context`, `EvalContext`, or `Environment`) should be injected into a callable; `from_obj` reads the marker attribute set by the decorators.
- **LRUCache**: A thread-safe `collections.abc.MutableMapping` implementing a fixed-capacity least-recently-used cache using a dict plus a deque, with pickling support and mapping-style access that updates recency on reads/writes.
- **Cycler**: Template helper that cycles through provided items across calls (`current`, `next`, `reset`), exposed to templates as `cycler`.
- **Joiner**: Callable helper that returns `""` the first time and a separator thereafter, useful for building delimited output in templates.
- **Namespace**: Attribute-style container backed by an internal dict, allowing templates/extensions to store arbitrary named values with attribute access and item assignment.

**Key functions and relationships**
- **pass_context / pass_eval_context / pass_environment**: Decorators that set `f.jinja_pass_arg` to a corresponding **_PassArg** value so the runtime can inject `jinja2.runtime.Context`, `jinja2.nodes.EvalContext`, or `jinja2.Environment` when calling the function.
- **internalcode**: Marks a function’s `CodeType` in the module-level `internal_code` set for internal tracking.
- **is_undefined**: Checks whether a value is an instance of `jinja2.runtime.Undefined`.
- **clear_caches**: Clears internal caches by calling `environment.get_spontaneous_environment.cache_clear()` and clearing `lexer._lexer_cache`.
- **import_string / open_if_exists / object_type_repr / pformat / consume**: General utilities for dynamic imports, conditional file opening, readable type naming, pretty formatting, and exhausting iterables.
- **urlize / url_quote / generate_lorem_ipsum / select_autoescape / htmlsafe_json_dumps**: Template-oriented helpers for linkifying text, URL quoting, generating placeholder text, choosing initial autoescape based on template name, and producing HTML-safe JSON via `markupsafe.Markup`.

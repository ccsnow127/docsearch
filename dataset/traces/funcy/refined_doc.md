# funcy Baseline Documentation

## Module: seqs

**purpose** This module provides a toolbox of sequence/iterator utilities for functional-style data processing, emphasizing lazy iteration (generators/itertools) with parallel “l*” helpers that eagerly materialize results as lists. It wraps Python’s built-ins and `itertools` to offer consistent behaviors (e.g., safe element access, splitting, grouping, partitioning, and reductions) and supports “function/predicate makers” via `make_func`/`make_pred` so callers can pass non-callable selectors/predicates.

**main functions**
- **generation and basic access**: `repeatedly` and `iterate` create infinite (or bounded) iterators; `take`/`drop` slice iterables; `first`/`second`/`nth`/`last` safely fetch elements from sequences or iterators; `rest` and `butlast` iterate all but the first/last element; `ilen` counts an iterable efficiently without storing it.
- **mapping/filtering variants**: `map`/`filter` extend the built-ins by normalizing `f`/`pred` through `make_func`/`make_pred`, while `lmap`/`lfilter` return lists; `remove`/`lremove` invert filtering via `itertools.filterfalse`; `keep`/`lkeep` map then keep truthy results (or, in one-argument form, filter truthy values); `without`/`lwithout` exclude specific values.
- **composition and reshaping**: `concat`/`lconcat` and `cat`/`lcat` chain sequences; `flatten`/`lflatten` recursively flatten nested containers based on `is_seqcont` (or a custom `follow` predicate); `mapcat`/`lmapcat` map then concatenate; `interleave` and `interpose` weave sequences and separators; `distinct`/`ldistinct` remove duplicates with optional keying.
- **splitting, grouping, counting**: `split` lazily bifurcates a stream into passing/failing iterators using internal buffering, while `lsplit` eagerly returns lists; `split_at`/`split_by` (and list forms) split by index or leading predicate-run; `group_by`, `group_by_keys`, and `group_values` build `defaultdict(list)` groupings; `count_by` and `count_reps` count occurrences into `defaultdict(int)`.
- **windowing/partitioning and adjacency**: `partition`/`chunks` (and list forms) cut data into fixed-size windows with a step, implemented by `_cut` choosing an optimized path for `collections.abc.Sequence` vs general iterators; `partition_by`/`lpartition_by` chunk by runs of equal key using `itertools.groupby`; `with_prev`, `with_next`, and `pairwise` provide neighbor-aware zips via `itertools.tee`; `lzip` provides a list-returning `zip`, emulating `strict=True` behavior on Python < 3.10.
- **reductions**: `reductions` yields intermediate accumulated values (delegating to `itertools.accumulate` when possible, otherwise `_reductions`), with `lreductions` as the list form; `sums`/`lsums` specialize reductions to partial sums using `operator.add`.

**relationships and external types** The module consistently pairs lazy iterator-returning functions with eager list-returning `l*` counterparts, and many operations are thin wrappers around `itertools` primitives (`islice`, `chain`, `tee`, `groupby`, `accumulate`, `takewhile`, `dropwhile`). Several APIs accept flexible selector/predicate inputs normalized by `make_func`/`make_pred`, and `flatten` relies on `is_seqcont` to decide which values are treated as nested sequence containers.

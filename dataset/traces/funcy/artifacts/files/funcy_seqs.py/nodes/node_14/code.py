...
# Capture built-ins at import time
_map = builtins.map
...
def make_pred(pred: Any) -> Callable[[Any], bool]:
    ...
    if isinstance(pred, str):
        def _truthy_attr(x, _a=pred):
            return bool(getattr(x, _a))
        return _truthy_attr
...
def _flatten_cycle_safe(seq, follow):
    active: set[int] = set()
    ...
    for item in seq:
        yield from _rec(item)


def flatten(seq, follow=is_seqcont):
    return _flatten_cycle_safe(seq, follow)

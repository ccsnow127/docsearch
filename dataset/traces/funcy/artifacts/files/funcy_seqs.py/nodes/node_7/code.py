...
def make_func(f: Any) -> Callable:
    ...
    if isinstance(f, (tuple, list)) and len(f) == 2 and f[0] == "attr":
        return operator.attrgetter(f[1])
    if isinstance(f, (tuple, list)) and len(f) == 2 and f[0] == "item":
...
def make_pred(pred: Any) -> Callable[[Any], bool]:
    ...
    if isinstance(pred, int):
        ...
        return _p
    if isinstance(pred, (tuple, list)) and len(pred) == 2 and pred[0] == "attr":
        ...
        return _p
    if isinstance(pred, (tuple, list)) and len(pred) == 2 and pred[0] == "item":
        getter = operator.itemgetter(pred[1])
...
def partition_by(f, seq):
    f = make_func(f)
    for _, items_iter in itertools.groupby(seq, f):
        yield items_iter

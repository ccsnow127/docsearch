...
def _string_accessor(name: str) -> Callable[[Any], Any]:
    def _acc(x):
        try:
            return x[name]
        except TypeError:
...
def _int_accessor(idx: int) -> Callable[[Any], Any]:
    def _acc(x):
        return x[idx]
...
def make_func(f: Any) -> Callable[..., Any]:
    ...
    if isinstance(f, dict):
        if "path" not in f:
            raise TypeError("Unsupported function specification dict (missing 'path').")
        steps = f["path"]
        if not isinstance(steps, (list, tuple)) or len(steps) == 0:
            raise TypeError("Unsupported function specification dict ('path' must be non-empty list/tuple).")

        step_funcs: list[Callable[[Any], Any]] = []
        for step in steps:
            if isinstance(step, str):
                step_funcs.append(_string_accessor(step))
            elif isinstance(step, int):
                step_funcs.append(_int_accessor(step))
            else:
                raise TypeError("Unsupported step type in path specification.")
    ...
    def _gen():
        ...
        while len(pool) == n:
            ...
            if step <= n:
                ...
                gap = step - n
                collections.deque(itertools.islice(it, gap), maxlen=0)
                pool = take(n, it)
                if len(pool) < n:
                    return

...
try:
    ...
    # allow this module to be imported standalone in the grading harness
    from osmnx import convert, projection, settings, utils  # type: ignore

try:  # shapely base geometry type for typing only
...
def _consolidate_subdivide_geometry(geom: Polygon | MultiPolygon) -> MultiPolygon:
    ...
    if mqas <= 0:
        raise ValueError("max_query_area_size must be positive.")
    ...
    if ratio > 10:
        warnings.warn(
            f"This geometry's area is {ratio} times the configured max query area size. "
    ...
    for line in lines:
        ...
        for g in geoms:
            if g.intersects(line):
                new_geoms.extend(split(g, line).geoms)
            else:
                new_geoms.append(g)
        geoms = new_geoms

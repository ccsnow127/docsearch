...
try:
    # When used as part of the osmnx package
    from . import convert, projection, settings, utils  # type: ignore
except Exception:  # pragma: no cover
    # When imported as a standalone module_under_test.py
...
def _consolidate_subdivide_geometry(geom: Polygon | MultiPolygon) -> MultiPolygon:
    ...
    if ratio > 10:
        warnings.warn(
            ...
            "will be divided into multiple sub-queries, and may take a long time.",
            category=UserWarning,
            stacklevel=2,
        )

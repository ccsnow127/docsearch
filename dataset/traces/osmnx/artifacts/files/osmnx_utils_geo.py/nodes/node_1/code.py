...
try:
    # When this module is part of the osmnx package
    from . import convert, projection, settings, utils  # type: ignore
except Exception:  # pragma: no cover
    # When executed as a standalone module_under_test.py
...
def _consolidate_subdivide_geometry(geom: Polygon | MultiPolygon) -> MultiPolygon:
    ...
    if ratio > 10:
        warnings.warn(
            f"This geometry's area is {ratio} times the configured maximum query area size. "
    ...
    for line in lines:
        ...
        for g in geoms:
            if g.intersects(line):
                ...
                new_geoms.append(g)
        geoms = new_geoms  # type: ignore[assignment]

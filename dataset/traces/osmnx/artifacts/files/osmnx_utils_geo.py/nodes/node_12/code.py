...
def buffer_geometry(geom: Geometry, dist: float) -> Geometry:
    ...
    geom_buff = geom_proj.buffer(dist)
    geom_buff_ll, _ = projection.project_geometry(geom_buff, crs=crs_proj, to_latlong=True)
    return geom_buff_ll
...
def sample_points(G: "nx.MultiGraph", n: int) -> "gpd.GeoSeries":
    ...
    edges = convert.graph_to_gdfs(G, nodes=False)[["geometry", "length"]]
...
def _consolidate_subdivide_geometry(geom: Polygon | MultiPolygon) -> MultiPolygon:
    ...
    if g.area > mqas:
        ...
        def _collect_polys(obj: Any) -> None:
            if obj is None:
                return
            ...
            if isinstance(obj, MultiPolygon):
                for p in obj.geoms:
                    if isinstance(p, Polygon) and (not p.is_empty):
                        parts.append(p)
                return
            geoms_attr = getattr(obj, "geoms", None)
            if geoms_attr is not None:
                for sub in geoms_attr:
                    _collect_polys(sub)

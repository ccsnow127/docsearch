...
try:
    # If this module is imported as part of the osmnx package.
    from . import projection, convert, settings, utils  # type: ignore
except Exception:  # pragma: no cover
    # If this file is imported standalone (as in the grading harness), fall back
    # to absolute imports from the osmnx package.
    from osmnx import projection, convert, settings, utils  # type: ignore
...
def _consolidate_subdivide_geometry(geom: Polygon | MultiPolygon) -> MultiPolygon:
    ...
    if geom.area > mqas:
        subdiv = _quadrat_cut_geometry(geom, quadrat_width=float(np.sqrt(mqas)))

        parts: list[Polygon] = []

        def collect_polys(g: Any) -> None:
            if g is None:
                return
            ...
            if isinstance(g, MultiPolygon):
                for p in g.geoms:
                    if not p.is_empty:
                        parts.append(p)
                return
            subgeoms = getattr(g, "geoms", None)
            if subgeoms is not None:
                for sg in subgeoms:
                    collect_polys(sg)

        collect_polys(subdiv)
    ...
    raise RuntimeError("Unexpected geometry type after consolidation/subdivision.")

...
def buffer_geometry(geom: Geometry, dist: float) -> Geometry:
    bbox_proj, crs_proj = projection.project_geometry(geom)
    buffered = bbox_proj.buffer(dist)
    buffered_ll, _ = projection.project_geometry(buffered, crs=crs_proj, to_latlong=True)
    return buffered_ll
...
def sample_points(G: "nx.MultiGraph", n: int) -> "gpd.GeoSeries":
    ...
    chosen_idx = rng_edges.choice(edges.index.to_numpy(), size=n, replace=True, p=weights)

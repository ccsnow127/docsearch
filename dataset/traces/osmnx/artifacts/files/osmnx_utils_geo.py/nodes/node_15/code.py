...
try:
    ...
    raise ImportError("geopandas is required for this module") from exc

try:
    from shapely.geometry import LineString, MultiLineString, MultiPolygon, Polygon
    from shapely.ops import split
except Exception as exc:  # pragma: no cover
    raise ImportError("shapely is required for this module") from exc


# Optional external dependencies expected by the documented API.
# These are imported lazily/optionally to allow test harnesses to inject stubs.
try:  # pragma: no cover
...
def _require_module(mod: Any, name: str) -> Any:
...
def buffer_geometry(geom: Geometry, dist: float) -> Geometry:
    proj = _require_module(projection, "projection")
    geom_proj, crs_proj = proj.project_geometry(geom)
    geom_buff_proj = geom_proj.buffer(dist)
    geom_buff, _ = proj.project_geometry(geom_buff_proj, crs=crs_proj, to_latlong=True)
    return geom_buff
...
def sample_points(G: nx.MultiGraph, n: int) -> gpd.GeoSeries:
    ...
    conv = _require_module(convert, "convert")
    edges = conv.graph_to_gdfs(G, nodes=False)[["geometry", "length"]]
...
def _consolidate_subdivide_geometry(geom: Polygon | MultiPolygon) -> MultiPolygon:
    ...
    st = _require_module(settings, "settings")
    mqas = st.max_query_area_size
    ...
    # Should be unreachable given the rules above.
    raise TypeError("Unexpected geometry type after consolidation/subdivision.")  # pragma: no cover
...
def bbox_from_point(
    ...
    if project_utm:
        ...
        bbox_proj, crs_proj = proj.project_geometry(bbox_poly)

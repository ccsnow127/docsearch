...
# Optional external modules expected by the documented behavior.
# They are referenced at runtime; tests/users should provide them.
try:
...
def _consolidate_subdivide_geometry(geom: Polygon | MultiPolygon) -> MultiPolygon:
    ...
    if hull_applied:
        ...
        if ratio > 10:
            warnings.warn(
                f"Geometry area is {ratio} times the maximum query area size; "
    ...
    if g.area > mqas:
        ...
        def _collect_polygons(x: Any) -> None:

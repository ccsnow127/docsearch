...
def sample_points(G, n: int):
    ...
    rng_edges = _np.random.default_rng()
    chosen = rng_edges.choice(gdf_edges.index.to_numpy(), size=n, replace=True, p=weights)

    lines = gdf_edges.loc[chosen, "geometry"]
    rng_pos = _np.random.default_rng()
...
def _consolidate_subdivide_geometry(geom: Polygon | MultiPolygon) -> MultiPolygon:
    ...
    if g.area > mqas:
        subdiv = _quadrat_cut_geometry(g, quadrat_width=np.sqrt(mqas))

        parts: list[Polygon] = []

        def _collect_polys(x: Any) -> None:
        ...
        _collect_polys(subdiv)

        if not parts:
            raise ValueError("Subdivision produced no polygonal parts after normalization.")
...
def _intersect_index_quadrats(geoms, polygon: Polygon | MultiPolygon) -> set[Any]:

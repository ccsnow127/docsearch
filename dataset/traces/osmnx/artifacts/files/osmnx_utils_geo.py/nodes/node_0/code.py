...
try:
    ...
    LineString = MultiLineString = MultiPolygon = Point = Polygon = None  # type: ignore
...
def buffer_geometry(geom: Geometry, dist: float) -> Geometry:
    projected_geom, crs_proj = projection.project_geometry(geom)
    buffered_projected = projected_geom.buffer(dist)
    buffered_latlon, _ = projection.project_geometry(buffered_projected, crs=crs_proj, to_latlong=True)
...
def sample_points(G: "nx.MultiGraph", n: int) -> "gpd.GeoSeries":
    ...
    total = lengths.sum()
    weights = lengths / total if total != 0 else np.full_like(lengths, 1 / len(lengths), dtype=float)
...
def _consolidate_subdivide_geometry(geom: Polygon | MultiPolygon) -> MultiPolygon:
    ...
    ratio = int(geom.area / mqas) if mqas != 0 else 0
    if ratio > 10:
        warnings.warn(
            f"This area is {ratio} times the configured max query area size and will be divided into multiple sub-queries and may take a long time.",
            UserWarning,
            stacklevel=2,
        )

    if geom.area > mqas:
        geom = _quadrat_cut_geometry(geom, quadrat_width=float(np.sqrt(mqas)))
...
def _quadrat_cut_geometry(geom: Polygon | MultiPolygon, quadrat_width: float) -> MultiPolygon:
    ...
    geoms: list[Polygon | MultiPolygon] = [geom]
    for line in lines:
        new_geoms: list[Polygon | MultiPolygon] = []
        for g in geoms:

...
try:
    from shapely.geometry.base import BaseGeometry as Geometry  # type: ignore
except Exception:  # pragma: no cover
    Geometry = Any  # type: ignore


# Optional external dependencies expected by contract
try:  # pragma: no cover
    from . import projection as projection  # type: ignore
except Exception:  # pragma: no cover
    try:
        import projection as projection  # type: ignore
    except Exception:
        projection = None  # type: ignore

try:  # pragma: no cover
    from . import convert as convert  # type: ignore
except Exception:  # pragma: no cover
    try:
        import convert as convert  # type: ignore
    except Exception:
...
try:  # pragma: no cover
    ...
    try:
        ...
        class _SettingsFallback:
            max_query_area_size = float("inf")

        settings = _SettingsFallback()  # type: ignore

try:  # pragma: no cover
    from . import utils as utils  # type: ignore
except Exception:  # pragma: no cover
    try:
        ...
        utils = _UtilsFallback()  # type: ignore


def buffer_geometry(geom: Geometry, dist: float) -> Geometry:
    geom_proj, crs_proj = projection.project_geometry(geom)  # type: ignore[attr-defined]
    geom_buff_proj = geom_proj.buffer(dist)
    geom_buff, _ = projection.project_geometry(  # type: ignore[attr-defined]
        geom=geom_buff_proj, crs=crs_proj, to_latlong=True
    )
    return geom_buff
...
def sample_points(G: nx.MultiGraph, n: int) -> gpd.GeoSeries:
    ...
    edges = convert.graph_to_gdfs(G, nodes=False)[["geometry", "length"]]  # type: ignore[attr-defined]
    lengths = edges["length"].to_numpy()
    weights = lengths / lengths.sum()

    rng_edges = np.random.default_rng()
    chosen_idx = rng_edges.choice(edges.index.to_numpy(), size=n, p=weights)

    lines = edges.loc[chosen_idx, "geometry"]
...
def interpolate_points(
    geom: LineString | MultiLineString, dist: float
...
def _consolidate_subdivide_geometry(geom: Polygon | MultiPolygon) -> MultiPolygon:
    ...
    mqas = settings.max_query_area_size  # type: ignore[attr-defined]
    hull_applied = False
    ...
    if geom.area > mqas:
        subdivided = _quadrat_cut_geometry(geom, quadrat_width=float(np.sqrt(mqas)))

        parts: list[Polygon] = []

        def _collect_polygons(g: Any) -> None:
            ...
            if isinstance(g, MultiPolygon):
                ...
                return
            if hasattr(g, "geoms"):
                for member in getattr(g, "geoms"):
                    _collect_polygons(member)

        _collect_polygons(subdivided)
...
def _intersect_index_quadrats(
    ...
    rtree = geoms.sindex
    utils.log(f"Built r-tree spatial index for {len(geoms):,} geometries", level="INFO")  # type: ignore[attr-defined]

    quadrat_width = max(0.1, float(np.sqrt(polygon.area)) / 10)
    multipoly = _quadrat_cut_geometry(polygon, quadrat_width)
    utils.log(
        f"Accelerating r-tree with {len(multipoly.geoms)} quadrats", level="INFO"  # type: ignore[attr-defined]
    )
    ...
    utils.log(
        f"Identified {len(geoms_in_poly):,} geometries inside polygon", level="INFO"  # type: ignore[attr-defined]
    )
...
def bbox_from_point(
    ...
    *,
    ...
    if project_utm:
        ...
        bbox_proj, crs_proj = projection.project_geometry(bbox_poly)  # type: ignore[attr-defined]
        bbox = bbox_proj.bounds

    utils.log(f"Created bbox {dist} meters from {point}: {bbox}", level="INFO")  # type: ignore[attr-defined]

...
# This module is designed to be importable both as part of the `osmnx`
# package and as a standalone file in this kata environment.
try:  # pragma: no cover
    from . import convert, projection, settings, utils  # type: ignore
except Exception:  # pragma: no cover
    from osmnx import convert, projection, settings, utils  # type: ignore
...
def interpolate_points(geom: LineString | MultiLineString, dist: float) -> Iterator[tuple[float, float]]:
    ...
    for n in range(num_vert + 1):
        f = n / num_vert
        pt: Point = geom.interpolate(f, normalized=True)
        yield (pt.x, pt.y)
...
def _consolidate_subdivide_geometry(geom: Polygon | MultiPolygon) -> MultiPolygon:
    err_msg = "Geometry must be a shapely Polygon or MultiPolygon."
...
def bbox_from_point(
    ...
    top = lat + float(delta_lat)
    bottom = lat - float(delta_lat)
    right = lon + float(delta_lon)
    left = lon - float(delta_lon)
    ...
    if project_utm:
        ...
        bbox = tuple(bbox_proj.bounds)  # type: ignore[assignment]

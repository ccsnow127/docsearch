# utils_geo Baseline Documentation

## Module: utils_geo

**Purpose**: Provides geospatial helper functions used across OSMnx for buffering geometries in meter units, sampling/interpolating points along network geometries, subdividing large polygons for efficient spatial operations, and creating/handling bounding boxes.

**Key functions and roles**
- **buffer_geometry**: Buffers an unprojected Shapely geometry by a meter distance by projecting to a meter-based CRS, applying the buffer, then reprojecting back to lat/long.
- **sample_points**: Produces a uniform random sample of points constrained to a projected, typically undirected `networkx.MultiGraph` by weighting edge selection by edge length (via `convert.graph_to_gdfs`) and interpolating random positions along chosen edge geometries.
- **interpolate_points**: Yields evenly spaced `(x, y)` coordinates along a `shapely.LineString`/`MultiLineString` using normalized interpolation based on the geometry’s length and a target spacing distance.
- **_consolidate_subdivide_geometry**: Prepares large projected `Polygon`/`MultiPolygon` query geometries by optionally taking a convex hull and subdividing them into smaller polygons when area exceeds `settings.max_query_area_size`.
- **_quadrat_cut_geometry**: Implements the polygon subdivision used by other helpers by recursively splitting a (Multi)Polygon with a grid of vertical/horizontal `LineString` “quadrat” lines (using `shapely.ops.split`) and returning a `MultiPolygon`.
- **_intersect_index_quadrats**: Accelerates intersection tests between a `geopandas.GeoSeries` and a (Multi)Polygon by using the GeoSeries r-tree spatial index (`geoms.sindex`) and intersecting against quadrat-cut polygon chunks, returning the set of intersecting GeoSeries index labels.
- **bbox_from_point**: Computes a `(left, bottom, right, top)` bounding box around a `(lat, lon)` point for a meter distance using spherical approximations, optionally projecting the bbox to UTM via `projection.project_geometry` (and optionally returning the projected CRS).
- **bbox_to_poly**: Converts bbox coordinates into a Shapely `Polygon`, primarily to support projection and other polygon-based operations.

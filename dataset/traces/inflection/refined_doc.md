# inflection Baseline Documentation

## Module: __init__

**Purpose**: Provides a Python inflection utility set modeled after Rails’ inflector, focused on converting words between singular/plural forms and transforming identifiers into common naming styles for code, titles, and URLs. It defines and maintains global inflection rule tables (**PLURALS**, **SINGULARS**) plus an **UNCOUNTABLES** set, and applies these rules via regex-based transformations using the standard library (**re**, **unicodedata**, **typing**).

**Key functions**
- **_irregular(singular, plural)**: Internal helper that prepends case-aware regex rules into **PLURALS** and **SINGULARS** for irregular word pairs; it is invoked at module load to register built-ins like person→people and child→children.
- **pluralize(word)** / **singularize(word)**: Convert between grammatical number by scanning **PLURALS**/**SINGULARS** in order and applying the first matching regex; both respect **UNCOUNTABLES** to leave certain words unchanged.
- **camelize(string, uppercase_first_letter=True)** / **underscore(word)**: Convert between snake_case and CamelCase (with optional lowerCamelCase), forming a loose inverse pair used by other helpers.
- **tableize(word)**: Produces Rails-style table names by applying **underscore** then **pluralize** (e.g., model/class name to plural snake_case).
- **humanize(word)** / **titleize(word)**: Create human-readable labels; **humanize** strips a trailing “_id”, replaces underscores with spaces, and normalizes casing, while **titleize** builds on **underscore** and **humanize** to capitalize words for display.
- **dasherize(word)** / **parameterize(string, separator='-')**: URL- and slug-oriented transforms; **dasherize** swaps underscores for dashes, and **parameterize** uses **transliterate** then regex cleanup to produce a lowercase, separator-delimited ASCII slug.
- **transliterate(string)**: Normalizes Unicode (NFKD) and strips non-ASCII characters to support ASCII-only outputs used by **parameterize**.
- **ordinal(number)** / **ordinalize(number)**: Compute English ordinal suffixes (“st”, “nd”, “rd”, “th”) and append them to numbers, with correct handling of 11/12/13 and negative values.

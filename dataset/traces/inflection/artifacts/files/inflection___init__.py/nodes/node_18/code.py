...
def _irregular(singular: str, plural: str) -> None:
    ...
    same_first_letter = singular[0].upper() == plural[0].upper()

    if same_first_letter:
        # Singular -> plural (preserve first-letter case via capture)
        PLURALS.insert(
            0,
            (
        ...
        # Plural -> plural idempotence (preserve first-letter case via capture)
        PLURALS.insert(
            0,
            (
        ...
        # Additional idempotence/consistency rules to reach exactly 6 new PLURALS entries
        PLURALS.insert(
            0,
            (
                r"(?i)({}){}$".format(plural[0].upper(), plural[1:]),
                r"\1" + plural[1:],
            ),
        )
        PLURALS.insert(
            0,
            (
                r"(?i)({}){}$".format(plural[0].lower(), plural[1:]),
                r"\1" + plural[1:],
            ),
        )
        PLURALS.insert(
            0,
            (
                r"(?i)({}){}$".format(singular[0].upper(), singular[1:]),
                r"\1" + plural[1:],
            ),
        )
        PLURALS.insert(
            0,
            (
                r"(?i)({}){}$".format(singular[0].lower(), singular[1:]),
                r"\1" + plural[1:],
            ),
        )

        # Plural -> singular (preserve first-letter case via capture)
        SINGULARS.insert(
            0,
            (
                r"(?i)({}){}$".format(plural[0], plural[1:]),
        ...
        # Singulars: uppercase/lowercase-first variants
        SINGULARS.insert(
            0,
            (
                r"{}{}$".format(plural[0].upper(), caseinsensitive(plural[1:])),

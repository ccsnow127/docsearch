...
def unidecode(value):
    ...
    for ch in s:
        ...
        if ch in ("ß",):
            ...
            # Drop other non-ASCII deterministically.
            pass
    return "".join(out)
...
def slugify(
    ...
    if allow_unicode:
        ...
        if isinstance(tval, str):
            text = tval
        elif isinstance(tval, (bytes, bytearray, memoryview)):
            b = tval if isinstance(tval, bytes) else bytes(tval)
            text = b.decode("utf-8", errors="replace")
        else:
            text = str(tval)
    ...
    if entities:
        ...
        text = ENTITY_PATTERN.sub(_ent_sub, text)

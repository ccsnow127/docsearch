# slugify Baseline Documentation

## Module: slugify

**Purpose**: Provides utilities to convert arbitrary text into URL-/filename-safe “slugs” by normalizing Unicode, decoding HTML character references, filtering disallowed characters, collapsing separators, optionally removing stopwords, and optionally truncating to a maximum length.

**Key functions**
- **smart_truncate(string, max_length=0, word_boundary=False, separator=" ", save_order=False)**: Trims a string to `max_length`, optionally respecting word boundaries based on `separator`; when `save_order` is true, it stops at the first word that would exceed the limit rather than skipping ahead.
- **slugify(text, ..., max_length=0, word_boundary=False, separator='-', save_order=False, stopwords=(), regex_pattern=None, lowercase=True, replacements=(), allow_unicode=False)**: Produces the final slug by applying optional user replacements, quote handling, Unicode normalization (and transliteration via `unidecode` when `allow_unicode` is false), optional decoding of HTML named/decimal/hex entities, lowercasing, numeric comma cleanup, regex-based character filtering, duplicate-separator collapsing, optional stopword removal, and then delegates truncation to **smart_truncate** before swapping the internal `'-'` separator to the requested `separator`.

**Relationships and external types**: **slugify** is the primary entry point and calls **smart_truncate** for length limiting; it relies on `re.Pattern`/`re.sub` for filtering, `unicodedata.normalize` for normalization, `html.entities.name2codepoint` for named entity decoding, and `unidecode.unidecode` (or `text_unidecode`) for ASCII transliteration when Unicode output is not allowed.
